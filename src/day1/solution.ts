...
function calculateCalibrationValue(line: string | number): number {
  const digits =
    typeof line === "number"
      ? line.toString().split("")
      : line.split("").filter((char: string) => /\d/.test(char));
...
const calibrationValuesPt1 = data.map((line) =>
...
type DigitKey = keyof typeof spelledOutDigits;

function calculateCalibrationValuePt2(line: string | number): number {
  if (typeof line === "number") {
    return calculateCalibrationValue(line);
  }

  const includedSpelledNumbers = Object.keys(spelledOutDigits).reduce(
    (acc: string[], cur) => {
  ...
  const orderedNumbers = [...includedSpelledNumbers, ...digits]
    ...
    .map(({ string }) => {
      return isNaN(Number(string))
        ? spelledOutDigits[string as DigitKey]
        : Number(string);
    });
...
const calibrationValuesPt2 = data.map((line) =>
  calculateCalibrationValuePt2(line)
);
