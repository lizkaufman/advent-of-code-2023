...
function countColors(draw: string) {
...
function checkGameAgainstTarget(
  countedColors: CountedColors,
  target: CountedColors
) {
  return (
    countedColors.red <= target.red &&
...
const possibleGames = testData.reduce(
  (acc: number[], cur: GameData): number[] =>
    checkGameAgainstTarget(cur.countedColors, targetPt1)
      ? [...acc, cur.gameId]
      : acc,
  []
);
